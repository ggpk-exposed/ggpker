import { Convert, File, IndexResponse } from "./index-response";
...
export default {
  ...
  async fetch(request, env, ctx): Promise<Response> {
    ...
    if (!path || path.endsWith("/")) {
      return show_dir(env.INDEX, path.substring(0, path.length - 1), adapter, version);
    } else {
...
const qsConf: IParseBaseOptions = {
  ...
  decoder: (v, def, cs, t) => (t === "value" && parseInt(v)) || def(v, def, cs),
};
...
async function show_dir(host: string, path: string, adapter: string, version: string): Promise<Response> {
  const result = await index("index", host, path, adapter);
...
async function show_file(extractor: string, file: File, req: Request): Promise<Response> {
  ...
  const last_block = Math.floor((file.bundle_offset! + file.size! - 1) / BLOCK_SIZE);
  ...
  for (let i = 0; i < block_count; i++) {
    ...
    if (i > last_block) {
      ...
      const end = i !== last_block ? extracted : (file.bundle_offset! + file.size!) % BLOCK_SIZE;
  ...
  if (i !== file.size) {
    console.warn("expected", file.size, "bytes, got", i);
  }
