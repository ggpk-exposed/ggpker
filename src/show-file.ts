...
import {current_version} from "./db";

const textDecoder = new TextDecoder("utf-16");

export async function show_file(env: Env, file: File, req: Request): Promise<Response> {
	if (!file.file_size) {
		console.warn("empty file requested, returning empty response", file);
		return new Response();
	}

	let cdn_url = await current_version(file.storage === "poe1" ? "1" : "2", env);
	if (!cdn_url.endsWith("/")) {
		cdn_url = cdn_url + "/";
	}
	...
	if (file.mime_type?.startsWith("text/") && file.extension !== "hlsl") {
